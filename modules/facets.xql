...
(:Sort facets by number of items (descending order):)
...
(:Display the facets and their value:)
...
(: Changed according to the code of the Eltec project + Addition to create collapsible panels :)
...
(:Original function to display facets:)
(:declare function facets:display($config as map(*), $nodes as element()+) {
    let $params := request:get-parameter("facet-" || $config?dimension, ())
    let $table := facets:print-table($config, $nodes, (), $params)
    where $table
